...
def add_rank(df):
    df["Rank"] = df.groupby("Date")["predict_target"].rank(ascending=False, method="first") - 1 
...
def add_combine_rank(df,percent_weight,com_matrix):
    ...
    for percent_ in percent_weight:
        temp_df['percent_info'] = temp_df['lag_rank_{PORTFOLIO_SIZE}']*percent_ + temp_df['predict_target']*(1-percent_)
...
for time_set in date_list:
    ...
    for sector in sector_set:
        ...
        test_data['predict_target'] = model.predict(x_test)
    ...
    # add_combine_rank(tool_dataframe,combine_percent,combine_matrix)
    print(">>>>>>>>>>>>>>")
...
print(lala_3.mean())
plt.show()
for combine_list in combine_matrix:
    plt.plot(tool_set,ratio_set)
    plt.plot(tool_set,random_ratio_set)
    plt.plot(tool_set,prerank_set)
    plt.plot(tool_set,combine_list)
